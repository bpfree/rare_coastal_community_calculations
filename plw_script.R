...
### 4. Export data
st_write(obj = plw_level0, dsn = paste0(plw_dir, "/", "plw_level0.shp"), append = F)
st_write(obj = plw_level1, dsn = paste0(plw_dir, "/", "plw_level1.shp"), append = F)
write.csv(plw_communities, file = paste0(plw_dir, "/", "plw_communities.csv"))
...
plw_level1_polyline <- st_cast(x = plw_level1_wgs84, to = "MULTILINESTRING")
...
### 9. Export data
st_write(obj = plw_10km, dsn = paste0(plw_dir, "/", "plw_10km.shp"), append = F)
st_write(obj = plw_10km_coast, dsn = paste0(plw_dir, "/", "plw_10km_coast.shp"), append = F)

######################################################
######################################################

### 10. Import coast data
plw_coast10km_wgs84 <- st_read(dsn = plw_dir, layer = "plw_10km_coast") %>%
  st_transform(crs)
...
st_write(obj = plw_level1_coast, dsn = paste0(plw_dir, "/", "plw_level1_coastal_communities.shp"), append = F)
...
### 16. Save tables
write.csv(plw_lvl1, file = paste0(plw_dir, "/", "plw_level1_summary.csv"))
...
plw_summary_table <- as.data.frame(plw_rare_count) %>%
...
write.csv(plw_summary_table, file = paste0(plw_dir, "/", "plw_country_summary_table.csv"))
